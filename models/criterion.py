...
class SetCriterion(nn.Module):
    ...
    def __init__(
        ...
        self.use_perturbed = use_perturbed
        self.use_non_perturbed = use_non_perturbed
    
    def similarity_loss(self, output_feats, target_feats):
        loss = torch.tensor([0.0]).to(output_feats.device)
        if self.use_perturbed:
            # print('p_l', torch.norm(output_feats - target_feats['perturbed_queries'], dim = -1).mean())
            loss += torch.norm(output_feats - target_feats['perturbed_queries'], dim = -1).mean()
        if self.use_non_perturbed:
            # print('non_p_l', torch.norm(output_feats - target_feats['non_perturbed_queries'], dim = -1).mean())
            loss += torch.norm(output_feats - target_feats['non_perturbed_queries'], dim = -1).mean()
        
        # print('perturbation_div', torch.norm(target_feats['perturbed_queries'] - target_feats['non_perturbed_queries'], dim = -1).mean())
        
        return {'sim_loss': loss}
    def loss_labels(self, outputs, targets, indices, num_masks, mask_type):
        ...
        num_samples = (target_classes != 253).sum()
        ...
        # self.noise_sum += perturbation.sum()
        # self.num_samples += num_samples
        # print(self.noise_sum)
        # print(self.noise_sum/self.num_samples)
    ...
    def forward(self, outputs, targets, p_output, mask_type):
        ...
        losses.update(self.similarity_loss(outputs['query_features'], p_output))
