...
import functools
from perturbed_feats_extractor import P_Mask3D
...
class InstanceSegmentation(pl.LightningModule):
    def __init__(self, config):
        ...
        weight_dict.update({"sim_loss": self.config.general.sim_loss_weight})
        ...
        self.perturbed_model = P_Mask3D(model_config_path = self.config.general.model_config_path, 
                                        checkpoint_path = self.config.general.checkpoint_path, 
                                        perturbation_magnitude = self.config.general.perturbation_magnitude, 
                                        perturbation_type=self.config.general.perturbation_type,
                                        use_perturbed = self.config.general.use_perturbed,
                                        use_non_perturbed = self.config.general.use_non_perturbed,)
    def forward(
    ...
    def training_step(self, batch, batch_idx):
        ...
        try:
            ...
            p_output = self.perturbed_model(data, target, raw_coordinates)
            # if self.global_step == 0:
            #     self.diff = 0
            #     self.num = 0
            # self.diff += torch.norm(p_output['perturbed_queries'] - p_output['non_perturbed_queries'], dim = -1).sum()
            # self.num += 500
            
            # if self.global_step%10 == 0:
            #     print(self.diff/self.num)
        ...
        try:
            losses = self.criterion(output, target, p_output, mask_type=self.mask_type)
        ...
        if self.config.data.test_mode != "test":
            ...
            try:
                losses = self.criterion(
                    output, target,p_output, mask_type=self.mask_type
                )
            except ValueError as val_err:
                print(f"ValueError: {val_err}")
                print(f"data shape: {data.shape}")
    ...
    def eval_instance_epoch_end(self):
        ...
        try:
            ...
            if self.validation_dataset.dataset_name == "scannet200":
                ...
                # print("mAP:",overall_ap_results[0])
                # print("mAP50:",overall_ap_results[1])
                # print("mAP25:",overall_ap_results[2])
                # exit()
                ap_results = {
